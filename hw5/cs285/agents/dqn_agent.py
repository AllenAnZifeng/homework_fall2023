...
class DQNAgent(nn.Module):
    ...
    def compute_critic_loss(
        ...
        # Compute the Q values from the current observation
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                # In Double Q-Learning, select best action according to the current critic
                next_actions = self.critic(next_obs).max(1)[1].unsqueeze(1)
                next_q_values = next_qa_values.gather(1, next_actions).squeeze(1)
            else:
                next_q_values = next_qa_values.max(1)[0]
