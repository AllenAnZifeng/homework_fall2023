...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          size = params['train_batch_size'] # 100

          indices = np.random.permutation(len(replay_buffer.acs))[:size]
          obs = replay_buffer.obs[indices]
          acs = replay_buffer.acs[indices]
          ob_batch, ac_batch = obs, acs
        ...
        if log_video:
            ...
            eval_video_paths = utils.sample_n_trajectories(
                env, actor, MAX_NVIDEO, MAX_VIDEO_LEN, True) # change back to True
